...
function simpleKeywordMatch(query: string, items: LostItem[]): string[] {
  ...
  items.forEach(item => {
    const searchText = `${item.title} ${item.description} ${item.location} ${item.category} ${item.features?.join(' ') || ''}`.toLowerCase()
    let score = 0
    ...
    words.forEach(word => {
      ...
      if (item.location.toLowerCase().includes(word)) score += 3
      if (item.category.toLowerCase().includes(word)) score += 2
      if (item.features?.some(f => f.toLowerCase().includes(word))) score += 4
    })
    ...
    const response = await fetch('https://api.deepseek.com/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `你是一个智能搜索助手。用户输入了一个查询，你需要判断这个查询与候选失物列表的匹配程度。

要求：
1. 只返回JSON对象，不要包含其他文字
2. 返回匹配度最高的Top 3物品ID，按匹配度从高到低排序
3. JSON格式：{"matches": [{"id": "物品ID", "score": 0-100的分数}]}
4. 非常重要：只返回物品列表中真实存在的物品ID，绝对不要编造任何ID！

评分标准（非常严格）：
- 完全匹配（多个关键词都匹配）：80-100分
- 有明显相关性（物品名称或关键描述匹配）：50-79分
- 只有一个弱相关关键词：20-49分
- 不匹配：0分

注意：
- 如果没有任何物品匹配，返回{"matches": []}
- 只返回分数>=50的物品（提高门槛，避免弱匹配）
- 只使用物品列表中提供的真实ID，不要编造
- 物品列表：${items.map(item => `ID:${item.id} | ${item.title} | ${item.description} | 地点:${item.location}`).join('\n')}
- 用户查询：${query}`
          },
          {
            role: 'user',
